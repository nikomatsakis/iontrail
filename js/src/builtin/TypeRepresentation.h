...
/*
  ...
  object is finalized, the TypeRepresentation* will be freed (and
  removed from the interning table, see below). Therefore, if you
  reference a TypeRepresentation*, you must ensure this dummy object
  is traced. In type objects, this is done by invoking
  TypeRepresentation::trace; in binary data type descriptors, the
  dummy object for the type is stored in `SLOT_TYPE_REPR`.
  ...
  The dummy object will eventually be exposed to self-hosted script
