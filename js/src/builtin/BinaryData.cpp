                       ...
                       uint8_t *mem)
{
    ...
    if (!GetPropertyNames(cx, fromObject, JSITER_OWNONLY, &ownProps)) {
        ReportCannotConvertTo(cx, from, typeRepr);
        return false;
    }
    ...
    for (size_t i = 0; i < typeRepr->fieldCount(); i++) {
        ...
        // Check that `fromObject` has `field.id` is an own property
        size_t j = 0;
        for (; j < ownProps.length(); j++) {
            if (ownProps[j] == field.id.get()) {
                break;
            }
        }
        if (j == ownProps.length()) {
            ReportCannotConvertTo(cx, from, typeRepr);
            return false;
        }
